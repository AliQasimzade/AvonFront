          ...
          <Container>
              ...
              <Col lg={12}>
                <div className="mb-4 pb-2">
                  <h5 className="mb-0 text-decoration-underline                                                                                                                                                                                                                                                                   ">
                    Order ID <b>#{searchOrder.codePrefix + searchOrder.codeNumber}</b>
                  </h5>
                </div>
              </Col>
            </Row>
            <div className="track-orders">
              <Row className="justify-content-between gy-4 gy-md-0">
                ...
                <Col
                  md={3}
                  className={`order-tracking text-start text-md-center ps-4 ps-md-0 ${searchOrder.status == "Qəbul" && 'completed'}`}
                >
                  <span className="is-complete"></span>
                ...
                <Col
                  md={3}
                  className={`order-tracking text-start text-md-center ps-4 ps-md-0 ${searchOrder.status == "İmtina" && 'completed'}`}
                >
                  <span className="is-complete"></span>
                ...
                <Row className="g-3">
                  <Col lg={3} xs={6}>
                    ...
                    <h5 className="fs-14 mb-0">
                      #<span id="invoice-no">{searchOrder.codePrefix + searchOrder.codeNumber}</span>
                    </h5>
                  </Col>
                  <Col lg={3} xs={6}>
                    <p className="text-muted mb-2 text-uppercase fw-medium fs-12">
                    ...
                    <h5 className="fs-14 mb-0 d-flex gap-1 align-items-center">
                      <span id="invoice-date ">{new Date(searchOrder.createdAt).toLocaleDateString()}</span>
                      <small className="text-muted" id="invoice-time">
                     {new Date(searchOrder.createdAt).getHours()} : {new Date(searchOrder.createdAt).getMinutes()}
                      </small>
                    </h5>
                  </Col>
                  <Col lg={3} xs={6}>
              ...
              <Card.Body className="p-4">
                <Row className="g-3">
                  <Col xs={6}>
                    ...
                    <h6 id="billing-name">{orders.name} {orders.surname}</h6>
                  ...
                  <Col xs={6}>
                    ...
                    <h6 id="shipping-name">{orders.name} {orders.surname}</h6>
                    <p className="text-muted mb-1" id="shipping-address-line-1">
                     {searchOrder.streetAddres}, {searchOrder.apartment}, {searchOrder.city}
                    </p>
                    <p className="text-muted mb-1">
                      <span>Phone: +</span>
                      ...
                      {(searchOrder.orderItems || [])?.map((item, index) => {
                        return (
                          <tr key={index}>
                            ...
                            <td>{item.count}</td>
                            <td className="text-end">${Number(item.salePrice * item.count).toFixed(2)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                  ...
                  >
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className="text-end">- {searchOrder.orderItems.find(s => s.discountPrice > 0).discountPrice}%</td>
                      </tr>
                      <tr>
                        <td>Shipping Charge</td>
                        <td className="text-end">₼{searchOrder.deliveryAdress.price}</td>
                      </tr>
                      <tr className="border-top border-top-dashed fs-15">
                        <th scope="row">Total Amount</th>
                        <th className="text-end">₼{searchOrder.totalAmount + searchOrder.deliveryAdress.price}</th>
                      </tr>
                    </tbody>
                  </Table>
                </div>
