...
const Cardshop = () => {
    ...
    const countDown = (item) => {
        setProductcount((productData || []).map((count) => (count.id === item.id && count.num > 0) ? { ...count, num: item.num > 0 ? item.num - 1 : 0, Total: (item.num > 0 ? item.num - 1 : 0) * item.ItemPrice } : count));
    }
